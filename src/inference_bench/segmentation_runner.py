...
def run_segmentation_pytorch(
    ...
    *,
    ...
    if precision not in {"fp32", "fp16"} or (precision == "fp16" and resolved_device.type != "cuda"):
        raise ValueError("Segmentation fp16 is only supported as a CUDA experiment; use fp32 otherwise.")
    ...
    with torch.inference_mode():
        ...
        for _ in range(timed_iterations):
            output, host_ms, device_ms = measure_inference(lambda: raw_segmentation_tensor(model(inputs)), resolved_device)
            _synchronize(resolved_device)
...
def run_segmentation_onnx(
    ...
    *,
    ...
    for _ in range(timed_iterations):
        output, host_ms = elapsed_ms(lambda: _synchronized_onnx(session, feeds, spec.output_name, spec.output_shape, resolved_device))
        samples.append(host_ms)
    if output is None:
...
def _synchronized_onnx(session: ort.InferenceSession, feeds: dict[str, np.ndarray], output_name: str, expected_shape: tuple[int, ...], device: str) -> np.ndarray:
    output = _onnx_once(session, feeds, output_name, expected_shape)

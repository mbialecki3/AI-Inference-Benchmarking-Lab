...
class OnnxExportTests(unittest.TestCase):
    ...
    def test_mobilenet_v3_large_export_is_structurally_valid(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            output_path = Path(temporary_directory) / "mobilenet_v3_large.onnx"
            result = export_onnx_quietly("mobilenet_v3_large", output_path)

            exported_model = onnx.load(str(output_path))
            onnx.checker.check_model(exported_model)

        self.assertEqual(result.model_name, "mobilenet_v3_large")
        self.assertEqual(result.input_shape, (1, 3, 224, 224))
        self.assertGreater(result.artifact_size_bytes, 0)
